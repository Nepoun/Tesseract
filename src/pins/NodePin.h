...
enum class PinType { Int, Float, Bool, String, Preset };

using PinValue = std::variant<int, float, bool, std::string, Preset>;
...
struct NodePin {
    ...
    NodePin(int id, int node_id, const char* name, PinType type)
        : id(id), node_id(node_id), name(name), type(type) {}
};
